    ...
    <div className={`flex h-full font-sans overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#0a0a0a] text-[#e0e0e0]' : 'bg-[#f9f9f9] text-[#2d2d2d]'}`}>
      ...
      <div className={`w-80 flex flex-col border-r relative z-20 transition-colors duration-500 ${isDarkMode ? 'bg-[#0f0f0f] border-[#222]' : 'bg-[#f0f0f0] border-[#e0e0e0]'}`}>
         ...
         <div className="p-6 pb-4">
             <div className="flex items-center justify-between mb-8">
                 <h1 className={`text-2xl font-bold tracking-tight flex items-center gap-3 font-['Playfair_Display'] italic ${isDarkMode ? 'text-white' : 'text-[#1a1a1a]'}`}>
         ...
         <div className="flex-1 overflow-y-auto custom-scrollbar px-4 pb-4 space-y-2">
                 ...
                 >
                    <div className="flex flex-col gap-1.5">
                        <div className="flex justify-between items-baseline">
                             <h3 className={`text-base font-bold line-clamp-1 font-['Playfair_Display'] ${activeNoteId === note.id ? (isDarkMode ? 'text-white' : 'text-black') : (isDarkMode ? 'text-gray-400 group-hover:text-gray-200' : 'text-gray-600 group-hover:text-black')}`}>
                        ...
                        <div className={`flex items-center justify-between mt-2 pt-2 border-t opacity-0 group-hover:opacity-100 transition-opacity ${isDarkMode ? 'border-[#1a1a1a]' : 'border-gray-100'}`}>
                            ...
                            <div className="flex gap-2">
                                <button onClick={(e) => toggleFavorite(note.id, e)} className={`${isDarkMode ? 'text-gray-600' : 'text-gray-400'} hover:text-[#d4af37]`}>
                                    <Star size={12} />
                                </button>
                                <button onClick={(e) => deleteNote(note.id, e)} className={`${isDarkMode ? 'text-gray-600' : 'text-gray-400'} hover:text-red-400`}>
                                    <Trash2 size={12} />
                                </button>
                            </div>
                        </div>
             ...
             {filteredNotes.length === 0 && (
                 <div className={`text-center py-12 flex flex-col items-center gap-3 ${isDarkMode ? 'text-gray-700' : 'text-gray-400'}`}>
      ...
      <div className={`flex-1 flex flex-col relative overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#050505]' : 'bg-[#fdfbf7]'}`}>
         ...
         {activeNote ? (
             ...
             >
                {/* Minimalist Toolbar */}
                <div className="h-16 flex items-center justify-between px-8 md:px-16 shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className={`flex items-center gap-2 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`}>
                            <button onClick={() => updateNote(activeNote.id, { isFavorite: !activeNote.isFavorite })}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-8 md:px-16 pb-12 max-w-5xl mx-auto w-full">
                    ...
                    <input 
                        ...
                        placeholder="Tiêu đề..."
                        className={`
                            w-full bg-transparent text-4xl md:text-5xl font-bold outline-none border-none font-['Playfair_Display'] tracking-tight mb-8 mt-4 transition-colors
             ...
             <div className={`flex-1 flex flex-col items-center justify-center gap-8 ${isDarkMode ? 'text-gray-600' : 'text-gray-300'}`}>
                 <div className="relative group">
                     <div className="absolute inset-0 bg-[#d4af37]/10 blur-3xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                     <Book size={64} className={`relative ${isDarkMode ? 'text-gray-800' : 'text-gray-200'}`} strokeWidth={0.5} />
                 </div>
                 <div className="text-center space-y-3">
                    <h3 className={`text-xl font-['Playfair_Display'] italic ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Silence is a source of great strength.</h3>
